...
class PluginApi {
   ...
   *
   * For example, if you have a attribute on the `User` model called
   * `subscribe_to_newsletter` that you want to include in the request payload
   * when a user saves their Emails preferences (`/my/preferences/emails`), you
   * can use this API to signal to the `emails` preferences controller that it
   * should treat your attribute as saveable, like so:
   *
   * ```
